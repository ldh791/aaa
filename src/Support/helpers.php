...
function render_inline_reply_form(string $boardKey, string $threadId, string $parentReplyId = '', string $label = '댓글 작성', string $context = 'thread'): void
{
    ...
    <section id="<?= e($formId) ?>" class="inline-reply-form glass-card is-collapsed" data-toggle-panel>
        ...
        <form action="/post.php?board=<?= e($boardKey) ?>&thread=<?= e($threadId) ?>" method="post" enctype="multipart/form-data" class="stack-form compact-form" data-remember-post-form="compose">
            ...
            <label><span>게시물 비밀번호</span><input type="password" name="post_password" minlength="4" maxlength="100" placeholder="수정/삭제할 때 사용" required oninvalid="this.setCustomValidity('비밀번호를 입력해주세요.')" oninput="this.setCustomValidity('')" data-remember-password></label>
...
function render_mobile_reply_dock(string $boardKey): void
{
    ...
    <section id="mobile-reply-dock" class="mobile-reply-dock is-collapsed" data-mobile-reply-dock>
        ...
        <div class="mobile-reply-sheet glass-card" role="dialog" aria-modal="true" aria-labelledby="mobile-reply-title">
            ...
            <form action="/post.php" method="post" enctype="multipart/form-data" class="stack-form compact-form mobile-reply-form" data-mobile-reply-form data-remember-post-form="compose">
    ...
    <div class="manage-stack">
        <section id="<?= e($prefix) ?>-edit" class="mini-manage-form<?= $isUnlocked ? '' : ' is-collapsed' ?>" data-toggle-panel>
            ...
            <?php if (!$isUnlocked): ?>
                ...
                <form action="/manage_post.php?board=<?= e($boardKey) ?>&thread_id=<?= e($threadId) ?><?= $isReply ? '&reply_id=' . rawurlencode($postId) : '' ?>" method="post" class="stack-form compact-form" data-remember-post-form="verify">
            ...
            <?php else: ?>
                <form action="/manage_post.php?board=<?= e($boardKey) ?>&thread_id=<?= e($threadId) ?><?= $isReply ? '&reply_id=' . rawurlencode($postId) : '' ?>" method="post" class="stack-form compact-form">
                    ...
                    <input type="hidden" name="return_to" value="<?= e($returnTo) ?>">
                    <label class="readonly-field"><span>이름</span><input type="text" name="name_display" maxlength="30" value="<?= e((string) ($post['name'] ?? '')) ?>" readonly aria-readonly="true"></label>
                    <?php if (!$isReply): ?>
